...
/// Try to fuse simple elementwise producers (linalg.fill or elementwise
/// generic) into the tiled consumer. This function is conservative and only
/// fuses when the producer is a linalg::GenericOp with all-parallel iterators
/// and 1:1 indexing maps (elementwise), or linalg::FillOp producers.
///
/// It uses linalg::fuseProducerOfTensor when available. If your MLIR version
/// does not provide that helper, implement the equivalent by cloning +
/// remapping.
static LogicalResult tryFuseProducers(Operation *consumerOp,
                                      RewriterBase &rewriter) {
  // Only handle LinalgOps
  auto consumer = dyn_cast<linalg::LinalgOp>(consumerOp);
  if (!consumer)
    return failure();

  // For each operand that is a tensor value and has a defining op,
  // attempt to fuse the producer if it's elementwise/fill.
  for (OpOperand &operand : consumerOp->getOpOperands()) {
    ...
    // If producer is linalg.fill, attempt to fuse: replacing the use with
    // an inlined body (this API may vary).
    if (isa<linalg::FillOp>(def)) {
      // Use linalg convenience if exists.
      // NOTE: tileUsingSCF often produces tensor::InsertSlice or uses newstyle
      // tensor results; adapt if required.
      if (auto res = linalg::fuseProducerOfTensor(rewriter, def, consumerOp)) {
        (void)res;
        continue;
      }
    }

    // If producer is linalg.generic and is elementwise (all parallel and
    // yields tensors with the same shape), attempt fuse.
    if (auto prodGeneric = dyn_cast<linalg::GenericOp>(def)) {
      auto itTypes = prodGeneric.getIteratorTypesArray();
      bool allParallel = llvm::all_of(itTypes, [](StringRef s) {
        return s == getParallelIteratorTypeName();
      });
      if (!allParallel)
        continue;

      // Check simple 1:1 indexing maps: all map results are AffineDimExpr and
      // count equals rank. Conservative check.
      bool simpleElementwise = true;
      for (AffineMap m : prodGeneric.getIndexingMapsArray()) {
        for (AffineExpr e : m.getResults()) {
          if (!e.isa<AffineDimExpr>()) {
            simpleElementwise = false;
            break;
          }
        }
        if (!simpleElementwise)
          break;
      }
      if (!simpleElementwise)
        continue;

      // Attempt built-in fuse helper (if your MLIR has it).
      if (succeeded(linalg::fuseProducerOfTensor(rewriter, prodGeneric,
                                                 consumerOp))) {
        continue;
      }
      // Fallback: skip if fuse helper not available.
    }
  }
  return success();
}
...
// ----------------- Main pass body -----------------
void TileFuseVectorizePass::runOnOperation() {
  func::FuncOp f = getOperation();
  MLIRContext *ctx = &getContext();

  // Default tile sizes if not provided.
  if (tileSizes.empty())
    tileSizes = {128, 128}; // example block-level tile (M x N)

  // We'll collect target Linalg ops (e.g. matmul or generic) and process them.
  SmallVector<linalg::LinalgOp> targets;
  f.walk([&](linalg::LinalgOp op) {
    // Filter: choose matmul or named generic you want to tile.
    if (isa<linalg::MatmulOp>(op) || isa<linalg::GenericOp>(op)) {
      targets.push_back(op);
    }
  });

  // Process each target op (note: tiling may change the graph; operate on a
  // copy)
  for (linalg::LinalgOp target : targets) {
    // Build tiling options
    OpBuilder b(target);
    Location loc = target.getLoc();

    // Compute loop range values (useful for dynamic shapes if you need them)
    SmallVector<Value> loopRangeVals = computeLoopRangeValues(b, target);

    // Convert user-provided integer tileSizes to Value vector (constants)
    SmallVector<Value> tileSizeVals;
    tileSizeVals.reserve(tileSizes.size());
    for (int64_t s : tileSizes)
      tileSizeVals.push_back(createIndexConstant(b, loc, s));

    // Option A: use linalg::tileUsingSCF with constant tile sizes
    linalg::LinalgTilingOptions opts;
    // Depending on your MLIR version you may supply tile sizes either as ints
    // or as Values. The below chooses the Value-based API if available.
    opts.setTileSizes(tileSizeVals);

    // Attempt to tile.
    RewritePatternSet emptyPatterns(ctx);
    // tileUsingSCF returns FailureOr<LinalgTilingResult> in many versions.
    FailureOr<linalg::LinalgTilingResult> tileRes =
        linalg::tileUsingSCF(b, target, opts);

    if (failed(tileRes)) {
      // Could not tile; skip this op.
      continue;
    }

    // Replace the original op with the tiled op(s) (tileUsingSCF may do that)
    linalg::LinalgTilingResult result = *tileRes;

    // The tiled op is in result.tiledOp (or results.tiledLinalgOp depending on
    // version)
    Operation *tiledOp = result.tiledOp.getOperation();

    // Fuse producers into the tiled op where possible.
    PatternRewriter rewriter(ctx);
    rewriter.setInsertionPoint(tiledOp);
    (void)tryFuseProducers(tiledOp, rewriter);
    ...
    // patterns.
    {
      ...
      // vectorization patterns - may vary by MLIR version
      // mlir::vector::populateVectorToVectorCanonicalizationPatterns(patterns);
      (void)applyPatternsAndFoldGreedily(tiledOp->getParentOfType<ModuleOp>(),
                                         std::move(patterns));
    }
    ...
    // applyPatternsAndFoldGreedily.
    {
      ...
      // e.g. linalg::populateVectorizationPatterns(vecPatterns);
      (void)applyPatternsAndFoldGreedily(tiledOp->getParentOfType<ModuleOp>(),
                                         std::move(vecPatterns));
    }
  } // for each target
}
